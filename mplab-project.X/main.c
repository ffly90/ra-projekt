...
int uart_rainbow[LEDS][UART_L];     // UART values for each LED
int uart_off[UART_L];               // UART values for LED off
...
void __ISR(_ADC_VECTOR, IPL3AUTO) ADCHandler(void){
    adc_offset++;
    if(adc_offset == 10){
        PR1 = 938+ 938*49+(938*49/4095*ADC1BUF0); // max freq for new led 100Hz , min freq 2Hz
        adc_offset = 0;
    }
    IFS1bits.AD1IF = 0;
}
void create_rainbow(){
    ...
    for (i = 0; i<LEDS; i++){
        for (j = 0; j<LEDS; j++){
            if(i == ( uart_pos / UART_L )){// check if current uart_pos is at the activ LED
                buffer[i][j] = uart_rainbow[i][uart_pos % UART_L ]; // Write the data byte to the UART.
            }else{
                buffer[i][j] = uart_off[uart_pos % UART_L ]; // Write the data byte to the UART.
            }
        }
    }
}
...
void __ISR(_UART1_TX_VECTOR, IPL5SRS) display(){
        ...
        if (pos == (uart_pos / UART_L))
    {
        ...
        "sw $t0, U1TXREG"
        :
        : [uart_rainbow] "r" (uart_rainbow), [pos] "r" (pos), [eleven] "r" (UART_L), [uart_pos] "r" (uart_pos), [uart_off] "r" (uart_off)
        : "t0", "t1", "t2"
        );
    }
    else
    {
        ...
        "sw $t1, U1TXREG"
        :
        : [uart_off] "r" (uart_off), [eleven] "r" (UART_L), [uart_pos] "r" (uart_pos)
        : "t1"
        );
    ...
    uart_pos++;
    if(uart_pos >= UART_L*LEDS){ // end uart transmision
        uart_pos = 0;
